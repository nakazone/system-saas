 ...
 *
 * Run: railway run node database/run-add-estimated-value.js
 * Or:  node database/run-add-estimated-value.js (with .env)
 */
...
const sql = `ALTER TABLE leads ADD COLUMN estimated_value DECIMAL(10,2) DEFAULT NULL COMMENT 'Valor estimado do projeto'`;
...
async function main() {
  ...
  try {
    ...
    console.log('Column estimated_value added to leads.');
  } catch (err) {
    if (err.code === 'ER_DUP_FIELD_NAME') {
      console.log('Column estimated_value already exists. Nothing to do.');
    } else {
