/**
 * Pipeline Kanban v4 — Contacted → Stand By; new column order; Meeting light green.
 * Idempotent: safe to run multiple times.
 *
 * Run: node database/migrate-pipeline-kanban-v4.js
...
const KANBAN_STAGES = [
  ...
  { slug: 'meeting_scheduled', name: 'Meeting Scheduled', order_num: 2, color: '#90EE90', is_closed: 0 },
  { slug: 'quote_sent', name: 'Quote Sent', order_num: 3, color: '#9b59b6', is_closed: 0 },
  { slug: 'follow_up_1', name: 'Follow Up', order_num: 4, color: '#F1C40F', is_closed: 0 },
  { slug: 'stand_by', name: 'Stand By', order_num: 5, color: '#f39c12', is_closed: 0 },
...
async function main() {
  ...
  console.log('Migrating Kanban pipeline v4 (Stand By)…');
  ...
  try {
    ...
    await conn.commit();
    console.log(`OK — stages updated; ${moved} lead(s) remapped.`);
    const [summary] = await conn.query(
      `SELECT ps.slug, ps.name, ps.order_num, ps.color, COUNT(l.id) AS n
