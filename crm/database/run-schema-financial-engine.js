/**
 * Execute schema-financial-engine.sql
 * Run: node database/run-schema-financial-engine.js
 */
...
async function runSchema() {
  ...
  try {
    console.log('🚀 Executando schema-financial-engine.sql...');
    ...
    const sqlFile = path.join(__dirname, 'schema-financial-engine.sql');
    ...
    const [tables] = await connection.query(`
      ...
      WHERE TABLE_SCHEMA = ? 
      AND (TABLE_NAME LIKE '%financial%' OR TABLE_NAME LIKE 'expenses' OR TABLE_NAME LIKE 'receipts' OR TABLE_NAME LIKE 'payroll%')
      ORDER BY TABLE_NAME
    `, [config.database]);
